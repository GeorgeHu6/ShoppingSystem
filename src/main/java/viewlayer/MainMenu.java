    ...
    public User shoppingPage(InputStream is) throws InterruptedException {
        ...
        if (inputStr.equals("q")) {
            ...
            while (innerLoop) {
                ...
                //输入数量符合条件（大于0且小于库存数）
                else if (ControlToolClass.intQualify(inputStr, 1, tmp.get(choice-1).getQuantity().intValue())) {
                    curUser.getUserCart().addItem(tmp.get(choice-1), Long.parseLong(inputStr));
